...
pData <- read.csv("/users/bcarancibia/CUNY_IS608/lecture2/data/all_PLUTO_data.csv", header = TRUE, sep =",", na.strings=c(""," ","NA"), stringsAsFactors=FALSE)

as.data.frame(pData)

#super hackish and not proud could not figure out why import was in factors and would not translate to numeric, tried a lot of things
...
ggsave("/users/bcarancibia/CUNY_IS608/lecture2/yrBuilt_5yr_average.png", height=3, width=5)
...
flrVsYr <- condense(bin(yrBuilt,),bin(numFloor,1)) 
...
ggsave("/users/bcarancibia/CUNY_IS608/lecture2/Wartime_homes.png", height=3, width=5)
